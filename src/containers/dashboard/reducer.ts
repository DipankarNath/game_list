...
import {addNewPlayer, updateExistingPlayer, setFetchedPlayers} from '../../api/playerAPI';
...
export const setPlayers = createAsyncThunk(
    'dashboard/players/setPlayers',
    async (players: PlayerType[]) => {
        const response = await setFetchedPlayers([...players]);
        return {data: response.data, message: ''};
    }
);
...
export const counterSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(setPlayers.pending, (state) => {
                state.status = 'loading';
            })
            .addCase(setPlayers.fulfilled, (state, action) => {
                state.status = 'idle';
                state.data.playerList = [...action.payload.data];
            })
            .addCase(setPlayers.rejected, (state) => {
                state.status = 'failed';
            })
    },
});
